...
Geo.prototype = {
        ...
        if (geoObject.geoQuery instanceof GeoQuery) {
          ...
          for (const prop in geoObject.geoQuery) {
            if (geoObject.geoQuery.hasOwnProperty(prop)
              && this._findHelpers.hasOwnProperty(prop)
  ...
  _distance: function(lat1, lon1, lat2, lon2) {
    ...
    return this.EARTH_RADIUS * Math.acos(Math.sin(lat1) * Math.sin(lat2)
  ...
  _getOutRectangle: function() {
    return (arguments.length === 1)
      ? this._getOutRectangleNodes(arguments[1])
      : this._getOutRectangleCircle(arguments[0], arguments[1])
  },
  ...
  stopGeofenceMonitoring: function(geofenceName) {
    ...
    if (!self._trackedFences.length) {
      ...
      if (!self._mobilecheck()) {
        clearInterval(self.monitoringId)
      } else {
        navigator.geolocation.clearWatch(self.monitoringId)
      }
    }
